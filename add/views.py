...
def get_digits(request):
	def list2trix(numbers,m,n):
		return [[numbers[i * n + v] for v in range(n)] for i in range(m)]
	...
	if request.method == 'POST':
		...
		if form.is_valid():
			...
			try:
				...
				m1 = form.cleaned_data['rows1']
				n1 = form.cleaned_data['cols1']
				numbers2 = form.cleaned_data['numbers2']
				m2 = form.cleaned_data['rows2']
				n2 = form.cleaned_data['cols2']

				print(list2trix(numbers1, m1, n1))
				print(list2trix(numbers2, m2, n2))
				leftsm = spartrix(matrix=list2trix(numbers1, m1, n1))
				print(leftsm)
				rightsm = spartrix(matrix=list2trix(numbers2, m2, n2))
				print(rightsm)
				if operator == "Add":
					outsm = leftsm + rightsm
				...
				valid_output = True
				return render(request, 'add/orthproj_output.html', {
				'nbs': nbs,
				#'P_rows': P_rows,
				'P_cols': P_cols,
				#'P_numr': P_numr,
	...
	#Render form
		...
		return render(request, 'add/orthproj.html', {
		'page_name': page_name,
		'section': section,
		'form': form,
